//LIVE REROUTING ONLY WORKS IF YOU CAN RESET THE GRID TO COMPARE, FIRST THING TO DO TOMORROW IS TO MAKE A RESET FUNCTION THAT KEEPS LIGHTS AND ROADS AND CONTINUOUSLY GENERATED CARS IN THE SAME SPOTS
/*Timings:
Traveling lane: lane.timeToTravel(speed)
Leaving Queue: GridController.reactionTime + Light.travelTime
Skipping Queue: time+Light.travelTime
Skipping Queue if something already in intersection: GirdController.reactionTime + Light.travelTime
...
interface Car {
    ...
    path?: Light[];
    pathIndex: number;
    outputCreateEventHandler():Function;
    getNextLane():Lane;
    moveToLaneCreateEventHandler(): Function;
 ...
 */
function carFactory(id: number, light: Light, mpg: number, path: Light[]): Car {
    return {
        id: id,
        light: light,
        mpg: mpg,
            ...
            return () => {
                ...
                //otherwise the car is done and has reached its destination
                    ...
                    if(allFinished) {
                        ...
                        if(GridController.iterations != GridController.currentIteration) {
        ...
        moveToLightQueueCreateEventHandler(): Function {
            return () => {
                ...
                //else add to exit queue
                    ...
                    let tempLight: Light = this.light;
                    let tempActiveLane: Lane = tempLight.activeLane;
                    let tempOppositeLane: Lane = tempActiveLane.getOpposite();
 ...
 */
function setUpCars(total: number) {
    GridController.Cars = [];
...
function testImprovements() {
    ...
    for(i = 0; i<GridController.Cars.length; i++) {
        let tempCar = GridController.Cars[i];
        let difference = GridController.results[tempCar.id][0] - GridController.results[tempCar.id][GridController.currentIteration-1];
        sum+=difference;

    }

    console.log(`Average difference between iteration ${GridController.currentIteration} and the original run: ${sum/i}`);
}
...
function setUpTestCars(total: number) {
    GridController.Cars = [];
...
function setUpTestCars2(total: number) {
    GridController.Cars = [];
