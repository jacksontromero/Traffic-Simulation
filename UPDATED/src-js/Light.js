...
function lightFactory(id) {
    return {
        ...
        nextCarOppositeCreateEventHandler() {
            return () => {
                let time = GridController.masterTime;
                //finds opposite lane 
                let oppositeLane = this.activeLane.getOpposite();
...
function generateLights() {
    ...
    for (let i = 0; i < GridController.gridSize; i++) {
        for (let j = 0; j < GridController.gridSize; j++) {
            let tempLight = lightFactory(GridController.gridSize * i + j);
            tempLight.coords = Array(GridController.stepSize * (i) + GridController.buffer, GridController.stepSize * (j) + GridController.buffer);
...
function generateTestLights() {
    ...
    GridController.updateSize(gridSlider.value(), width);
    for (let i = 0; i < GridController.gridSize; i++) {
        let tempHorLight = lightFactory(i);
        tempHorLight.coords = Array(GridController.stepSize * (i) + GridController.buffer, width / 2);
        GridController.Lights.push(tempHorLight);
    }
    for (let i = 0; i < GridController.gridSize; i++) {
        let tempVertLight = lightFactory(GridController.gridSize + i);
        tempVertLight.coords = Array(width / 2, GridController.stepSize * (i) + GridController.buffer);
